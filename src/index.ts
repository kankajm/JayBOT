...
client.on("message", async (channel: any, context: any, message: any, self: any) => {
  ...
  if (message.startsWith("!whitelist add")) {
    let mcNick: string = message.replace("!whitelist add", "").replace(/\s+/g, '');
  ...
  if (admin_users.includes(sender_name)) {
    if (message == "!bot debug") {
      ...
      client.say(
        channel,
        `Bot version is: ${bot_version}, Node.JS version: ${process.version}, RCON STATUS: ${rconResponse}, Created with ❤️ by Jaroslav Kaňka 2021.`
      );
    }
